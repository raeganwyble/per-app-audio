...
// Themed single-select dropdown for the per-app audio panel. Drop-in
// compatible with the kit's qs.Ui Dropdown (same properties/signals), with one
// addition: `openUp` flips the popup to open above the trigger instead of
// below, so the list stays on screen when the panel's parent bar is at the
// bottom of the monitor (Pactl/pw-dump device names can push the list below
// the screen edge otherwise).
//
// Derived from the Omarchy shell Ui/Dropdown.qml — Copyright (c) omacom,
// MIT License (https://github.com/omacom/omarchy). Keep behavior aligned with
// the kit so the two stay visually identical.
Item {
  id: root
  ...
  // Open the popup above the trigger instead of below. Set by the plugin when
  // the bar anchors to the bottom edge of the screen.
  ...
  Column {
    ...
    BorderSurface {
      ...
      Popup {
        ...
        x: 0
        y: root.openUp
          ? -(popup.implicitHeight + Style.spacing.xxs)
